...
const { app, addEntrypoint } = createAgentApp(
  {
    ...
    version: "0.0.1",
    description:
      "CumBot - an accidental philosopher who lives in group chats.",
  },
  {
...
addEntrypoint({
  ...
  price: process.env.ENTRYPOINT_PRICE || "1.00", // Default to 1.00 USDC (or set via ENTRYPOINT_PRICE env var)
...
addEntrypoint({
  ...
  input: z
    .object({
      query: z.string().min(1, { message: "Provide a query (topic or @username)" }),
      platform: z.enum(["discord", "telegram"], { message: "Platform must be 'discord' or 'telegram'" }),
      channelId: z.string().optional().describe("Discord channel ID (required for Discord)"),
      chatId: z.string().optional().describe("Telegram chat ID (required for Telegram)"),
      serverId: z.string().optional().describe("Discord server/guild ID"),
      lookbackMinutes: z
        .coerce.number()
        .int()
        .min(1)
        .max(1440)
        .default(60)
        .describe("Number of minutes to look back for context"),
    })
    .superRefine((value, ctx) => {
      if (value.platform === "discord" && !value.channelId) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "channelId is required for Discord",
          path: ["channelId"],
        });
      }
      if (value.platform === "telegram" && !value.chatId) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
  ...
  async handler(ctx) {
    ...
    const queryType = query.startsWith("@") || query.match(/^<@\d+>/) ? "person" : "topic";
    
    let messages: SummarizerMessage[] = [];
    let chatContext = "";
    ...
    if (platform === "discord" && channelId) {
      ...
      const allMessages = getTelegramMessagesWithin(chatNumeric, lookbackMinutes);
    ...
    if (!chatContext && queryType === "topic") {
      chatContext = "No recent messages about this topic found.";
    } else if (!chatContext && queryType === "person") {
